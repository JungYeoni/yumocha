...
### 재발명 금지
- 새 전처리·분류 로직을 작성하기 전에 `src/features/`, 기존 노트북(`notebooks/`), `reports/`의 관련 계획서에 이미 구현된 함수·로직이 있는지 먼저 확인한다.
- 특히 세부사업명 텍스트 정제(`flatten_header`, `classify_row` 등 `20260712_시도별_예산현황_분리정제_작업계획.md` 기준 로직)는 여러 작업에서 공통으로 재사용해야 하는 대상이므로, 동일 목적의 로직을 별도로 새로 만들지 않는다.
- 기존 로직을 그대로 쓸 수 없는 이유(입력 구조 차이 등)가 있을 때만 새로 작성하고, 그 이유를 코드 주석 또는 계획서에 명시한다.
