...
모형화 판단이다. 계수(φ)는 log1p(F_i,t-1)이 1단위 증가할 때 TFR의 수준
변화다. F_i,t-1이 큰 관측치(F≫1)에서는 이게 근사적으로 "F 1% 변화당 φ×0.01
수준 변화"에 가깝지만, 세부영역 평균이 22원까지 내려가는 F_i,t-1이 작은
관측치에서는 이 근사가 성립하지 않는다 — 정확한 1% 변화 효과는
...
데이터 분할 정책: train/validation/test 분할이나 stratify를 쓰지 않는다.
이 스크립트는 예측 모델이 아니라 지역·연도 고정효과 회귀로 관측자료 수준의
관련성을 추정하는 추론(inference) 분석이라, 2016~2024년 전체 패널을 표본
확정(§ 결측 시차 제외) 후 그대로 추정에 쓴다. 시차 피처(F_i,t-1/-2)는 표본
확정 이전 단계에서 이미 만들어져 있다(build_subarea_fiscal_response_regression_sample.py).
"""
