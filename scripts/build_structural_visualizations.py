...
def prepare_visualization_input(
    ...
    *,
    ...
    indicator_regions = data.loc[data["지역"].ne("전국")].groupby("지표_id")["지역"].unique()
    for indicator_id, regions in indicator_regions.items():
        if set(regions) != set(expected_regions):
            raise ValueError(f"{indicator_id}의 17개 시도 구성이 올바르지 않습니다.")
