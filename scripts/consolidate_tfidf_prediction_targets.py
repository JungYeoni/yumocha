...
def save_outputs(
    ...
    """통합본·예측 핵심본·QA를 UTF-8-SIG CSV로 저장한다."""
    output_dir.mkdir(parents=True, exist_ok=True)
    paths = {
        "통합본": output_dir / "2016_2020_2022_2024_TFIDF_예측대상_통합.csv",
        "예측본": output_dir / "2016_2020_2022_2024_TFIDF_예측대상.csv",
        "QA": output_dir / "2016_2020_2022_2024_TFIDF_예측대상_QA.csv",
    }
    combined.to_csv(paths["통합본"], index=False, encoding="utf-8-sig")
    combined[PREDICTION_COLUMNS].to_csv(paths["예측본"], index=False, encoding="utf-8-sig")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/interim/영역분류_라벨링/예측대상"),
    )
